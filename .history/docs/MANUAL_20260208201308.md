...
1.  **Start Embedding Server (Port 8081):**
    ```bash
    ./lcm --run --config production.json --embedding --port 8081 --mode server
    ```

2.  **Start Reranker Server (Port 8082):**
    ```bash
    ./lcm --run --config production.json --re-ranker --port 8082 --mode server
    ```

3.  **Start Main Chat Server (Port 8080):**
    ```bash
    ./lcm --run --config production.json --chat --mode server
    ```
